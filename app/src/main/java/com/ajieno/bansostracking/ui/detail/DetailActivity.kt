        ...
        RetrofitClient.instance.detailresult(extradata)
            .enqueue(object : retrofit2.Callback<DataBansosResponse> {
                override fun onResponse(
                    ...
                    det_jtanggungjawab.text = response.body()?.jtanggungjawab.toString()
                    det_ibuhamil.text = response.body()?.ibuhamil.toString()
                    det_lanjutusia.text = response.body()?.lanjutusia.toString()
                    det_disabilitas.text = response.body()?.disabilitas.toString()
                    det_jmotor.text = response.body()?.jmotor.toString()
                    det_jmobil.text = response.body()?.jmobil.toString()
